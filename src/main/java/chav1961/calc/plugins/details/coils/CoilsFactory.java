...
public class CoilsFactory implements PluginInterface<CoilsPlugin>{
	private static final String	PLUGIN_NAME = "menu.details.coils"; 
	...
	@Override
	public CoilsPlugin newIstance(final LoggerFacade facade) {
		return new CoilsPlugin(facade);
	}
	...
	public ContentNodeMetadata getMetadata() {
		try {
			return new MutableContentNodeMetadata(getPluginName()
					...
					, null
					, CoilsPlugin.class.getAnnotation(LocaleResource.class).value()
					, CoilsPlugin.class.getAnnotation(LocaleResource.class).tooltip()
