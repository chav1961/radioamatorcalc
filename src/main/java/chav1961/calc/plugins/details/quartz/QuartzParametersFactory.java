...
public class QuartzParametersFactory implements PluginInterface<QuartzParametersPlugin> {
	private static final String	PLUGIN_NAME = "menu.details.quartzparameters"; 
	...
	@Override
	public QuartzParametersPlugin newIstance(final LoggerFacade facade) {
		return new QuartzParametersPlugin(facade);
	}
	...
	public ContentNodeMetadata getMetadata() {
		try {
			return new MutableContentNodeMetadata(getPluginName()
					...
					, null
					, QuartzParametersPlugin.class.getAnnotation(LocaleResource.class).value()
					, QuartzParametersPlugin.class.getAnnotation(LocaleResource.class).tooltip()
