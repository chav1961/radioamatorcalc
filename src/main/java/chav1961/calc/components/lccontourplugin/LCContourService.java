...
public class LCContourService implements PluginInterface {
	...
	public synchronized PluginInstance newInstance(final Localizer localizer, final LoggerFacade logger) throws LocalizationException, SyntaxException, ContentException, IOException {
		return new LCContour(localizer,new LCContourCalculator(localizer,logger)); 
	}
	...
	public synchronized String[] getUsesIds(final Localizer parent) throws LocalizationException {
		if (inner == null) {
			try{inner = new LCContour(parent,new SystemErrLoggerFacade());
	...
	private static class LCContour extends AutoBuiltForm<LCContourCalculator> implements PluginInstance {
		...
		public LCContour(final Localizer localizer, final LoggerFacade logger) throws NullPointerException, IllegalArgumentException, LocalizationException, SyntaxException, ContentException, IOException {
			this(localizer,new LCContourCalculator(localizer,logger));
		}

		protected LCContour(final Localizer localizer, final LCContourCalculator instance) throws NullPointerException, IllegalArgumentException, LocalizationException, SyntaxException, ContentException, IOException {
