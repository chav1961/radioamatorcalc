...
public class PipeExecutor {
	...
	public void run() throws Exception {
		...
		try(final SimpleURLClassLoader	sucl = new SimpleURLClassLoader(new URL[0],Thread.currentThread().getContextClassLoader())) {
			
			for (int index = 0, maxIndex = temporaries.length; index < maxIndex; index++) {
				temporaries[index] = content[index].preparePipeItem(sucl);
			}
			try{
